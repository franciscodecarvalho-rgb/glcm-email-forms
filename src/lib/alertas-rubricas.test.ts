...
const cc = (competencia: string, itens: { codigo?: string | null; descricao?: string | null }[]) => ({
  id: `cc-${competencia}`,
  competencia,
...
describe("encontrarRubricasAlerta", () => {
  ...
  it("agrupa competências sem duplicar e conserva a primeira descrição", () => {
    const alertas = encontrarRubricasAlerta([
      cc("01/2024", [{ codigo: "1513", descricao: "BANCO DE HORAS" }]),
      cc("02/2024", [{ codigo: "1513" }, { codigo: "1513", descricao: "OUTRA" }]),
    ]);
    expect(alertas).toEqual([
      { codigo: "1513", descricao: "BANCO DE HORAS", competencias: ["01/2024", "02/2024"] },
    ]);
  });
