-- Migration: 20260915153000_deduplicacao_canonica_relatorios.sql
-- Aplica deduplicação canônica nos relatórios (evita duplicação cartesiana de holerites),
-- enriquece o relatório por pessoa com empresa, quantidade de competências e temas,
-- e preserva a consistência de permissões para autenticados e service_role.

DROP FUNCTION IF EXISTS public.relatorio_lancamentos_pessoa(text, jsonb, jsonb, text[], text, text, integer, integer);
DROP FUNCTION IF EXISTS public.relatorio_rubricas(jsonb, jsonb, text[], text, text, integer, integer);
DROP FUNCTION IF EXISTS public.relatorio_por_empresa(jsonb, jsonb, text[], text, text, integer, integer);
DROP FUNCTION IF EXISTS public.relatorio_por_pessoa(jsonb, jsonb, text[], text, text, integer, integer);
DROP FUNCTION IF EXISTS public.relatorio_total_geral(jsonb, jsonb, text[], text, text);
DROP FUNCTION IF EXISTS public.relatorio_totais_tema(jsonb, jsonb, text[], text, text);
DROP FUNCTION IF EXISTS public.relatorio_itens_filtrados(jsonb, jsonb, text[], text, text);
...
CREATE FUNCTION public.relatorio_itens_filtrados(
  ...
  base_bruta AS (
    SELECT i.id AS item_id, i.contracheque_id, c.caso_id,
           ...
           ROW_NUMBER() OVER (
             PARTITION BY
               CASE WHEN public.cpf_valido(cs.cpf)
                    THEN 'cpf:' || public.normalizar_cpf_digitos(cs.cpf)
                    ELSE 'caso:' || c.caso_id::text END,
               c.competencia,
               COALESCE(i.codigo, ''),
               i.tipo,
               public.normalizar_termo_tema(i.descricao)
             ORDER BY c.created_at DESC NULLS LAST, i.id
           ) AS rn
    ...
    WHERE i.valor > 0 AND i.tipo IN ('provento', 'desconto')
  ),
  base AS (
